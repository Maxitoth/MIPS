...
def argomenti_parse():
    ...
    parser.add_argument("-exh","--excel_hazards", help =" booleano per decidere se generare altri due fogli excel con un foglio avente gli hazards trovati e l'altro foglio con numero di cicli di clock, data e control hazard totali, cicli di clock in ogni parte del programma e istruzioni nella pipeline. Se inserito viene generato il file excel (Se viene inserito dopo -exp allora viene generato un unico excel con tutti e tre i fogli)", action= "store_true")
...
def avvia_simulatore(inputs):
    ...
    dizionario_hazards = {}
    dizionario_hazards["Data Hazards Trovati"] = simulazione[1]["Data Hazards Trovati"]
    dizionario_hazards["Control Hazards Trovati"] = simulazione[1]["Control Hazards Trovati"]
    simulazione[1].pop("Data Hazards Trovati")
    simulazione[1].pop("Control Hazards Trovati")
    json_object_pipeline = json.dumps(simulazione[0])
    json_object_hazards = json.dumps(dizionario_hazards)
    json_object_clocks = json.dumps(simulazione[1])
    ...
    simulatore.genera_excel(json_object_pipeline, json_object_hazards, json_object_clocks, sim.bool_forwarding, inputs.excel_pipeline, inputs.excel_hazards)
